...
// void gather_and_print(int N, int P, int rank, double *B_sendbuf, int sendcount, int *recvcounts, int *displs, double *B_global, int partition_type);
...
//             int remainder = N % P;
//             for(int p = 0; p < P; p++) {
//                 recvcounts[p] = (rows_per_proc + (p < remainder ? 1 : 0)) * N;
//             }
//             displs[0] = 0;
//             for(int p = 1; p < P; p++) {
...
//                     B_global, recvcounts, displs, MPI_DOUBLE,
//                     0, MPI_COMM_WORLD);
...
//     initialize_A_local_row(local_N, N, start_row, A_local);
...
//     initialize_A_local_checkerboard(local_rows, local_cols, start_row, start_col, A_local, N);
